 ...
 */
class Ttcdoc1 extends AActiveRecordSP
{
	...
	public function executeSp($mode,$type,&$transaction)
	{
		...
		if($this->error_code < 0)
			$this->addError('error_msg', 'Error 1t'.$this->error_code.' '.$this->error_msg);
	...
	public function executeSprGenTc()
	{
		...
		if($this->error_code < 0)
			$this->addError('error_msg', 'Error 2t'.$this->error_code.' '.$this->error_msg);
	...
	public function executeSpManyHeader($exec_status,$menuName,&$transaction)
	{
		...
		if($this->error_code < 0)
			$this->addError('error_msg', 'Error 3t'.$this->error_code.' '.$this->error_msg);
	...
	public function executeSpGenTcClob($tc_id,$tc_rev,$tc_clob_eng,$tc_clob_ind,$tc_matrix_eng,$tc_matrix_ind,$updstat)
	{
		...
		if($this->error_code < 0){
			//var_dump($this->error_code.' '.$this->error_msg);
			//die();
			$this->addError('error_msg', 'Error 4t '.$this->error_code.' '.$this->error_msg);
		}
