...
public static function checkJournalApproval($doc_date)
{
     $bgn_date = $doc_date;
     if(DateTime::createFromFormat('Y-m-d',$bgn_date)) $bgn_date = DateTime::createFromFormat('Y-m-d',$bgn_date)->format('Y-m-01');
     
    
    $sql="  SELECT COUNT(1)CNT  FROM(
            SELECT TO_DATE(FIELD_VALUE,'YYYY/MM/DD HH24:MI:SS')DOC_DATE FROM T_MANY_DETAIL D, T_MANY_HEADER H WHERE 
            D.UPDATE_SEQ=H.UPDATE_SEQ
            AND D.UPDATE_DATE =H.UPDATE_DATE
            AND D.TABLE_NAME='T_ACCOUNT_LEDGER'
            AND D.FIELD_NAME='DOC_DATE'
            AND D.RECORD_SEQ=1
            AND H.UPDATE_DATE>TRUNC(SYSDATE)-30
            AND H.APPROVED_STATUS='E'
            UNION ALL
            SELECT TO_DATE(FIELD_VALUE,'YYYY/MM/DD HH24:MI:SS') DOC_DATE FROM T_MANY_DETAIL D, T_MANY_HEADER H WHERE 
            D.UPDATE_SEQ=H.UPDATE_SEQ
            AND D.UPDATE_DATE =H.UPDATE_DATE
            AND D.TABLE_NAME='T_FUND_MOVEMENT'
            AND D.FIELD_NAME='DOC_DATE'
            AND D.RECORD_SEQ=1
            AND H.UPDATE_DATE>TRUNC(SYSDATE)-30
            AND H.APPROVED_STATUS='E'
            )
            WHERE DOC_DATE between to_date('$bgn_date','yyyy-mm-dd') and to_date('$doc_date','yyyy-mm-dd')";
