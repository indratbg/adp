...
class Rptclientstockposition extends ARptForm
{
	public $doc_date;
	...
	public function rules()
	{
		return array(
			...
			array('bond_flg,type_client,client_option,group_by,group_option,branch_cd,position_option,branch_option,rem_cd,sales_option,report_type,stk_option, stk_cd, dummy_date, bgn_client, end_client','safe')
		);
	}
...
public function cek_date()
{
	...
	public function executeRpt($bgn_stk, $end_stk, $bgn_client, $end_client, $bgn_rem, $end_rem, $bgn_branch, $end_branch, $position, $custody , $bgn_client_type_3,$end_client_type_3, $bgn_margin, $end_margin)
	{
		...
		try{
			$query  = "CALL SPR_STOCK_POSITION_INTERNAL(	TO_DATE(:P_DOC_DATE,'YYYY-MM-DD'),
														    :P_BGN_STK_CD,
														    :P_END_STK_CD,
														    :P_BGN_CLIENT,
														    :P_END_CLIENT,
			...
			$command->bindValue(":P_POSITION",$position,PDO::PARAM_STR);
			...
			$command->bindValue(":P_GROUP_BY",$this->group_by,PDO::PARAM_STR);
			$command->bindValue(":P_BOND_FLG",$this->bond_flg,PDO::PARAM_STR);
