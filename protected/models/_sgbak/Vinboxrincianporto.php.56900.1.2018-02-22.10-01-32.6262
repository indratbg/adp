...
class Vinboxrincianporto extends Tmanyheader
{
	public $report_date;
	...
	public function search()
	{
		$criteria = new CDbCriteria;
			...
			$criteria->select = "t.*, 
								(
									...
									AND field_name = 'REPORT_DATE'
									AND RECORD_SEQ=1
								) report_date";
			
			$criteria->join = "JOIN T_MANY_DETAIL a ON t.update_date = a.update_date AND t.update_seq = a.update_seq";
			$criteria->condition = "a.TABLE_NAME = 'LAP_RINCIAN_PORTO' AND a.FIELD_NAME = 'REPORT_DATE' AND record_seq = 1";
		...
		$sort = new CSort;
		$sort->defaultOrder='report_date desc';
