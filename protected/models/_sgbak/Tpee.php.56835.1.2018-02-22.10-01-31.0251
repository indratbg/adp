...
class Tpee extends AActiveRecordSP
{
	...
	public function executeSp($exec_status,$old_id)
	{
		...
		try{
			...
			$command->bindValue(":P_IPO_BANK_CD",$this->ipo_bank_cd,PDO::PARAM_STR);
			$command->bindValue(":P_IPO_BANK_ACCT",$this->ipo_bank_acct,PDO::PARAM_STR);
			$command->bindValue(":P_IPO_ACCT_NAME",$this->ipo_acct_name,PDO::PARAM_STR);
	...
	public function rules()
	{
		return array(
			...
			array('ipo_acct_name', 'length', 'max'=>50),
			array('jenis_penjaminan,ipo_bank_acct', 'length', 'max'=>30),
			//array('gl_acct_bank,gl_acct_utang,sl_acct_bank,sl_acct_utang','required','on'=>'penjatahan'),
			array('ipo_bank_cd,batch,journal_date,total_setor,user_id2,rdi_type,check_gl, ipo_bank_cd, ipo_bank_acct, client_cd,branch_cd,remarks,gl_acct_bank,gl_acct_utang,sl_acct_bank,sl_acct_utang,voucher_ref,total,gl_acct_bank,sl_acct_bank, gl_acct_utang, sl_acct_utang,tahap,option,tahap,stk_cd_temp,stk_cd_ksei,jurnal_distribdt,distrib_dt_to, cre_dt, upd_dt, paym_dt, tgl_kontrak, allocate_dt, upd_by', 'safe'),
	...
	public function attributeLabels()
	{
		return array(
			...
			'ipo_bank_cd' => 'Bank Penampung',
			'ipo_bank_acct' => 'Rekening Penampung',
			'ipo_acct_name' => 'Nama Rekening'
		);
	}
