...
class Tcontracts extends AActiveRecordSP
{
	...
	public function rules()
	{
		return array(
			...
			array('due_dt_for_amt, contr_dt, qty, price, stk_cd, client_cd, brok_perc','required'),
			array('broker_lawan_perc','required','on'=>'intrabroker'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
	
			array('sell_brokname,buy_brokname,broker_lawan_perc,cancel_flg,avg_price,adv_paymt_flg, adv_scrip_flg, amend_dt, kpei_due_dt, ksei_due_dt, amt_for_curr, brch_cd, brok, brok_perc, can_amd_flg, client_cd, client_type, commission, contr_dt, 
	...
	public function attributeLabels()
	{
		return array(
			...
			'sell_brokname' => 'Sell Broker Name',
			'buy_brokname' => 'Buy Broker Name'
		);
	}
	...
	public function executeSpIntra($exec_status)
	{
		...
		try{
			...
			$command->bindValue(":P_LAWAN_PERC",$this->broker_lawan_perc*100,PDO::PARAM_STR);
