...
class Tinterest extends AActiveRecordSP
{
	...
	public function rules()
	{
		return array(
			...
			array('int_flg, post_flg, ovr_flg, approved_stat', 'length', 'max'=>1,'except'=>'filter'),
			array('user_id, upd_by, approved_by', 'length', 'max'=>10),
			array('brch_cd,total_int_amt,total_compensation,total_pay_late,client_cd,save_flg,pay_late,compensation,journal_dt,int_dt_from,int_dt_to,year,bulan,type_desc,client_type,client_name,option_posting,cre_dt, upd_dt, trx_dt_beli, trx_dt_jual, approved_dt', 'safe'),
