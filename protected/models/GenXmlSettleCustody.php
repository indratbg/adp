...
Class GenXmlSettleCustody extends CFormModel
{
	...
	function executeSp()
	{
		...
		if($this->cekInsert == 'Y'){
			try{
				$query  = "CALL SP_INSERT_SETTLE_TUNAI(
	...
	function rules()
	{
		return array(
			...
			array('cekInsert, settle_date, client_cd, beli_jual, stk_cd, price, qty, trx_amt, trf_amt, custodian_cd, instruction_type, trf_qty, contr_num, sum_flg, net_flg, trading_ref','safe')
		);
	}
