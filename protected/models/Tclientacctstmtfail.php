...
class Tclientacctstmtfail extends AActiveRecordSP
{
	...
	{	//var_dump($this->cifs);die();
		$sql = "SELECT B.BANK_ACCT_NUM,B.CLIENT_CD, COUNT(B.BANK_ACCT_NUM) CNT FROM MST_CLIENT A, MST_CLIENT_FLACCT B
			    WHERE  A.CLIENT_CD=  B.CLIENT_CD  AND B.ACCT_STAT <> 'C'  AND A.CIFS = '$this->cifs' AND A.CLIENT_CD='$this->client_cd'
			    GROUP BY B.BANK_ACCT_NUM, b.client_cd";
		$check = DAO::queryAllSql($sql);
		if(!$check)
			$this->addError('client_cd', 'Invalid client code '.$this->client_cd);
		$this->cekRDI();
	}
	public function cekRDI()
	{
		$check = Clientflacct::model()->find("client_cd='$this->client_cd' and acct_stat <> 'C' and bank_acct_num='$this->acct_num' and approved_stat='A' ");
		if(!$check)
			$this->addError('client_cd', 'Nomor rekening dana nasabah tidak ditemukan '.$this->client_cd);
	}
	public static function getData($from_dt, $to_dt, $branch_cd)
	{
		$check = DAO::queryRowSql("select dstr1 from mst_sys_param where param_id='HOST TO HOST' AND PARAM_CD1='NOTIF' AND PARAM_CD2='YJ'");
		$closing_hour = $check['dstr1'];
		...
		$sql=" SELECT 
		      ...
		      --CASE WHEN TRX_DATE> TO_DATE(TO_CHAR(TRUNC(TRX_DATE),'YYYY-MM-DD')||' $closing_hour','YYYY-MM-DD HH24:MI:SS') THEN 
		      --GET_DUE_DATE(1,TRUNC(TRX_DATE)) 
		      --WHEN F_IS_HOLIDAY(TO_CHAR(TRUNC(TRX_DATE),'DD/MM/YYYY'))=1 THEN
		      --GET_DUE_DATE(1,TRUNC(TRX_DATE))
		      --ELSE
			  --TRUNC(TRX_DATE) 
			  --END TANGGALEFEKTIF,
			  trunc(sysdate) as TANGGALEFEKTIF,
		      ...
		      M.CIFS
		    FROM
		      (SELECT A.external_ref,acct_num,trx_date,trx_type, trx_cd,acct_debit,acct_credit,trx_amt
		    ...
		    AND A.trx_date  BETWEEN GET_DOC_DATE(1,TO_DATE('$from_dt','dd/mm/yyyy')) AND GET_DUE_DATE(1,TO_DATE('$to_dt','dd/mm/yyyy'))
		     ...
		     (SELECT  MAX(CLIENT_CD)CLIENT_CD,BANK_ACCT_NUM
     			FROM MST_CLIENT_FLACCT WHERE ACCT_STAT <>'C'  AND BANK_CD='BCA02'
