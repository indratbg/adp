...
class Taccountledger extends AActiveRecordSP
{
	...
	{
		$this->old_curr_val = $this->curr_val;
	...
	public function rules()
	{
		...
		//	array('sl_acct_cd','checkRequired'),
			...
			array('netting_flg, sett_status, approved_sts', 'length', 'max'=>1),
			...
			array('rowid, cash_withdraw_amt, cash_withdraw_reason, db_cr_flg,curr_val,bank_flg,system_generated,old_tal_id,old_xn_doc_num,cancel_flg,save_flg,cre_dt, upd_dt, doc_date, due_date, netting_date, approved_dt, arap_due_date', 'safe'),
	...
	public function checkRequired()
	{
		...
		if($check)
		{
			if($check->prm_desc2 == 'Y' && !$this->sl_acct_cd)$this->addError('sl_acct_cd', 'SL Account must be filled');
		}
		else {
			if(!$this->sl_acct_cd)$this->addError('sl_acct_cd', 'SL Account must be filled');
		}
	}
	*/
	public function ceksl_a(){
		...
		if($this->gl_acct_cd != 'NA' )
		{
			...
			$sql="select trim(gl_a),trim(sl_a) from mst_gl_account where acct_stat='A' and approved_stat='A' and sl_a=trim('$this->sl_acct_cd')";
	...
	public function cekgl_a(){
		...
		if($this->gl_acct_cd != 'NA')
		{
			$sql="select gl_a from mst_gl_account where acct_stat='A' and approved_stat='A' and  gl_a='$this->gl_acct_cd'";
			$cek=DAO::queryRowSql($sql);
...
*/
	...
	public function executeSp($exec_status,$old_xn_doc_num,$old_tal_id,$update_date,$update_seq,$record_seq)
	{
		...
		try{
			...
			$command->bindValue(":P_CASH_WITHDRAW_AMT",$this->cash_withdraw_amt,PDO::PARAM_STR);
			$command->bindValue(":P_CASH_WITHDRAW_REASON",$this->cash_withdraw_reason,PDO::PARAM_STR);
