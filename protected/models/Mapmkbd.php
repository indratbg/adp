...
class Mapmkbd extends AActiveRecordSP
{
	...
	public function cekgl_a(){
		...
		if($this->source =='VD51' || $this->source =='VD52'  ){
	
		
		$sql = "select distinct gl_a from MST_GL_ACCOUNT where GL_A = '$this->gl_a'";
	...
	else if($this->source =='VD57'){
		$sql = "select distinct gl_acct_cd from MST_SECURITIES_LEDGER where gl_acct_cd = '$this->gl_a'";
		$cek_gl_a = DAO::queryRowSql("$sql");
