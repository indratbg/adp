...
class Tpayrecd extends AActiveRecordSP
{
	...
	{ 
		$sql = "SELECT  DECODE(b.doc_ref_num, NULL,b.xn_doc_num,b.doc_ref_num) contr_num,		
				...
				b.FOLDER_CD ref_folder_cd,  b.TAL_ID, DECODE(SUBSTR(b.record_source,2,3),'DUE',b.netting_flg,b.tal_id) doc_tal_id,
				 b.RECORD_SOURCE, b.cre_dt sortkey1, b.xn_doc_num gl_ref_num, b.due_Date, budget_cd, brch_cd		
		 		FROM
		 		( 
		 			SELECT GREATEST(acct_open_dt -30,to_date('$startDt','yyyy-mm-dd')) AS begin_dt		
				 	FROM MST_CLIENT 
		 		...
		 		AND due_date <= to_date('$voucherDt','yyyy-mm-dd')
