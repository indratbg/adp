...
class Tjvchh extends AActiveRecordSP
{
	...
	public function rules()
	{
		return array(
			...
			array('save_flg,client_cd,jur_date,file_upload,rtn,doc_num,doc_date,old_jvch_num,jvch_date, cre_dt, upd_dt, approved_dt, int_adjust', 'safe'),
	...
	public function check_amt()
	{
		if($this->curr_amt>1000000000000)
		{
			$this->addError('curr_amt', 'Amount greater than 1,000,000,000,000');
		}
	}
...
public function checkFolderCd()
	{
		...
		if(!$this->isNewRecord){
			...
			$olf_folder_cd = Tjvchh::model()->find("jvch_num= '$this->jvch_num' ")->folder_cd;
			
			if($olf_folder_cd == $this->folder_cd){
				$folderChange = false;
			}
