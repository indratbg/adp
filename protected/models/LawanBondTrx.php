...
class LawanBondTrx extends AActiveRecordSP
{
	...
	public function rules()
	{
		return array(
			...
			array('lawan,lawan_name,lawan_type,capital_tax_pcn,deb_gl_acct,cre_gl_acct','required'),
			array('capital_tax_pcn', 'numerical'),
			array('lawan', 'length', 'max'=>12),
			array('lawan_name, contact_person', 'length', 'max'=>50),
			array('lawan_type, approved_sts, participant, is_active', 'length', 'max'=>1),
			...
			array('cre_dt, upd_dt, e_mail,approved_dt,ctp_cd,sl_acct_cd,custody_cbest_cd', 'safe'),

			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
	
			array('lawan, lawan_name, lawan_type, phone, fax, contact_person, capital_tax_pcn, deb_gl_acct, cre_gl_acct, sl_acct_cd, cre_dt, user_id, upd_dt, upd_by, approved_dt, approved_by, approved_sts,cre_dt_date,cre_dt_month,cre_dt_year,upd_dt_date,upd_dt_month,upd_dt_year,approved_dt_date,approved_dt_month,approved_dt_year,e_mail', 'safe', 'on'=>'search'),
		);
	}
...
public function executeSp($exec_status,$old_id)
	{
		...
		$command->bindValue(":P_PARTICIPANT",$this->participant,PDO::PARAM_STR);
		$command->bindValue(":P_IS_ACTIVE",$this->is_active,PDO::PARAM_STR);
	...
	public function search()
	{
		...
		$criteria->compare('lower(lawan)',strtolower($this->lawan),true);
		$criteria->compare('lower(lawan_name)',strtolower($this->lawan_name),true);
		$criteria->compare('lawan_type',$this->lawan_type,true);
		$criteria->compare('custody_cbest_cd',$this->custody_cbest_cd,true);
		...
		$criteria->compare('lower(sl_acct_cd)',strtolower($this->sl_acct_cd),true);
