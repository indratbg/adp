...
$this->breadcrumbs=array(
	'Deposit Client Inbox'=>array('index'),
	'Unprocessed Deposit Client',
);

$this->menu=array(
	
	array('label'=>'Unprocessed Deposit Client', 'itemOptions'=>array('style'=>'font-size:30px;font-weight:bold;color:black;margin-left:-17px;margin-top:8px;')),
	...
	array('label'=>'List','url'=>Yii::app()->request->baseUrl.'?r=finance/tclientdeposit/index','icon'=>'list','itemOptions'=>array('style'=>'float:right')),
);
		...
		'actionButtons' => array(
			...
			array(
				...
				'click' => 'js:function(checked_element){
						...
						showPopupModal("Reject Reason","'.(Yii::app()->getBaseUrl(true).'/index.php?r=inbox/tclientdeposit/AjxPopRejectChecked').'"+temp);	
				}'
			)
		),
		...
		array(
			...
			'buttons'=>array(
		        'approve'=>array(
		        	'label'=>'approve', 
		            'icon' =>'ok',                          
		            'url'  =>'Yii::app()->createUrl("/inbox/tclientdeposit/approve", array("id" => $data->primaryKey))',				// AH : change 
		         ),
		         'reject'=>array(
		         	'label'=>'reject',
		            'icon'=> 'remove',
		            'url' => 'Yii::app()->createUrl("/inbox/tclientdeposit/AjxPopReject", array("id" => $data->primaryKey))',			// AH : change
		            'click'=>'js:function(e){
		            	e.preventDefault();
