...
$this->menu=array(
	...
	array('label'=>'List','url'=>Yii::app()->request->baseUrl.'?r=custody/tpee/index','icon'=>'list'),
);
		...
		'actionButtons' => array(
			...
			array(
				...
				'click' => 'js:function(checked_element){
						...
						showPopupModal("Reject Reason","'.(Yii::app()->getBaseUrl(true).'/index.php?r=inbox/Tpee/AjxPopRejectChecked').'"+temp);	
				}'
			)
		),
		...
		array(
			...
			'buttons'=>array(
		        'approve'=>array(
		        	'label'=>'approve', 
		            'icon' =>'ok',                          
		            'url'  =>'Yii::app()->createUrl("/inbox/Tpee/approve", array("id" => $data->primaryKey))',				// AH : change 
		         ),
		         'reject'=>array(
		         	'label'=>'reject',
		            'icon'=> 'remove',
		            'url' => 'Yii::app()->createUrl("/inbox/Tpee/AjxPopReject", array("id" => $data->primaryKey))',			// AH : change
		            'click'=>'js:function(e){
		            	e.preventDefault();
