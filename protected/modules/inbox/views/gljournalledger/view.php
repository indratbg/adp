...
$this->menu=array(
	array('label'=>'View GL Journal Entry Inbox #'.$modelParentDetail->folder_cd, 'itemOptions'=>array('style'=>'font-size:30px;font-weight:bold;color:black;margin-left:-17px;margin-top:8px;')),
	...
	array('label'=>'Update','icon'=>'pencil','url'=>array('update&id='.$model->update_seq.''),'itemOptions'=>array('style'=>'float:right')),
);
?>
...
<?php
if(DateTime::createFromFormat('Y/m/d H:i:s',$modelParentDetail->jvch_date)){
	$modelParentDetail->jvch_date=DateTime::createFromFormat('Y/m/d H:i:s',$modelParentDetail->jvch_date)->format('d M Y');
}
else if(DateTime::createFromFormat('Y-m-d H:i:s',$modelParentDetail->jvch_date)){
	$modelParentDetail->jvch_date=DateTime::createFromFormat('Y-m-d H:i:s',$modelParentDetail->jvch_date)->format('d M Y');
}
else{
	$modelParentDetail->jvch_date;
}
?>
<div class="row-fluid">
	<div class="span4">
		<?php echo $form->label($modelParentDetail,'jvch_date',array('class'=>'control-label',
		'value'=>$modelParentDetail->jvch_date,'style'=>'font-weight:bold')); ?>
