...
<table id='table-data' class='table table-bordered table-condensed'>
	...
	?>
		<tr id="row<?php echo $x ?>">
			...
			<td style="text-align: right;"><?php echo number_format(($oldModelDetailSell[$x-1]->sisa_temp + $row->sisa_temp),0);?></td>
			<td style="text-align: right;"><?php echo number_format($row->sisa_temp,0);?></td>
			<td style="text-align: right;"><?php echo number_format($oldModelDetailSell[$x-1]->sisa_temp,0);?></td>
...
<h4>Data Bond Transaction</h4>
<div class="row-fluid">
	<div class="span6">
		<?php $this->widget('bootstrap.widgets.TbDetailView',array(
			'data'=>$oldModelDetailNew,
			'id'=>'tablecompare',
			'attributes'=>array(
				'trx_type',
				'trx_id',
				...
				array('name'=>'int_rate','value'=>$oldModelDetailNew->int_rate*100),
				array('name'=>'nominal','value'=>number_format($oldModelDetailNew->nominal,0)),
				array('name'=>'Price','value'=>$oldModelDetailNew->price.' %'),
				array('name'=>'cost','value'=>number_format($oldModelDetailNew->cost,0)),
			),
		)); ?>
		...
		<?php $this->widget('bootstrap.widgets.TbDetailView',array(
			...
			'attributes'=>array(
				'accrued_days',
				'accrued_int_round',
				array('name'=>'accrued_int','value'=>number_format($oldModelDetailNew->accrued_int,0)),
			),
		)); ?>
		...
		<?php $this->widget('bootstrap.widgets.TbDetailView',array(
			...
			'attributes'=>array(
				'accrued_tax_days',
				'accrued_tax_round',
				array('name'=>'accrued_tax_pcn','value'=>($oldModelDetailNew->accrued_tax_pcn*100).' %'),
				array('name'=>'accrued_int_tax','value'=>number_format($oldModelDetailNew->accrued_int_tax,0)),
				array('name'=>'capital_gain','value'=>number_format($oldModelDetailNew->capital_gain,0)),
				array('name'=>'capital_tax_pcn','value'=>($oldModelDetailNew->capital_tax_pcn*100).' %'),
				array('name'=>'capital_tax','value'=>number_format($oldModelDetailNew->capital_tax,0)),
				array('name'=>'multi_buy_price','value'=>$oldModelDetailNew->multi_buy_price == 'Y'?'Yes' : 'No'),
				array('name'=>'Purchase Price','value'=>$oldModelDetailNew->multi_buy_price == 'Y'?'-' : ($oldModelDetailNew->buy_price.' %')),
				array('name'=>'buy_dt','type'=>'date'),
				'buy_trx_seq',
				array('name'=>'buy_value_dt','type'=>'date'),
				array('name'=>'seller_buy_dt','type'=>'date'),
				array('name'=>'net_capital_gain','value'=>number_format($oldModelDetailNew->net_capital_gain,0)),
				array('name'=>'net_amount','value'=>number_format($oldModelDetailNew->net_amount,0)),
				'sign_by'
			),
		)); ?>
	</div>
