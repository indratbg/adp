...
$this->menu=array(
	...
	array('label'=>'Generate','url'=>Yii::app()->request->baseUrl.'?r=contracting/ttcdoc1/index','icon'=>'list'),
);
		...
		'actionButtons' => array(
			...
			array(
				...
				'click' => 'js:function(checked_element){
						...
						showPopupModal("Reject Reason","'.(Yii::app()->getBaseUrl(true).'/index.php?r=inbox/ttcdoc/AjxPopRejectChecked').'"+temp);	
				}'
			)
		),
		...
		array(
			...
			'buttons'=>array(
		        'approve'=>array(
		        	'label'=>'approve', 
		            'icon' =>'ok',                          
		            'url'  =>'Yii::app()->createUrl("/inbox/ttcdoc/approve", array("id" => $data->primaryKey))',				// AH : change 
		         ),
		         'reject'=>array(
		         	'label'=>'reject',
		            'icon'=> 'remove',
		            'url' => 'Yii::app()->createUrl("/inbox/ttcdoc/AjxPopReject", array("id" => $data->primaryKey))',			// AH : change
		            'click'=>'js:function(e){
		            	e.preventDefault();
