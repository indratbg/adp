...
$this->breadcrumbs=array(
	'Netting Voucher Inbox'=>array('index'),
	$model->update_seq,
);

$this->menu=array(
	array('label'=>'View Netting Voucher Inbox #'.$model->update_seq, 'itemOptions'=>array('style'=>'font-size:30px;font-weight:bold;color:black;margin-left:-17px')),
...
<table id='tableLedger' class='table table-bordered table-condensed'>
	...
	?>
		<tr id="row<?php echo $x ?>">
			...
			<td style="text-align:right"><?php if($row->db_cr_flg == 'D')echo Tmanydetail::reformatNumber($row->payrec_amt);else echo '0.00'; ?></td>
			<td style="text-align:right"><?php if($row->db_cr_flg == 'C')echo Tmanydetail::reformatNumber($row->payrec_amt);else echo '0.00'; ?></td>
			<td><?php echo $row->remarks ?></td>
		</tr>
	<?php $x++;} ?>
		<tr>
			<td colspan=4></td>
			<td style="text-align:right"><?php $debit=0; foreach($modelChildDetail as $row)if($row->db_cr_flg == 'D')$debit+=$row->payrec_amt;echo Tmanydetail::reformatNumber($debit); ?></td>
			<td style="text-align:right"><?php $credit=0; foreach($modelChildDetail as $row)if($row->db_cr_flg == 'C')$credit+=$row->payrec_amt;echo Tmanydetail::reformatNumber($credit); ?></td>
			<td></td>
		</tr>
	</tbody>
</table>
