...
$this->menu=array(
	...
	array('label'=>'List','url'=>Yii::app()->request->baseUrl.'?r=glaccounting/tvd55/index','icon'=>'list'),
);
...
<h1>Unprocessed MKBD VD55 XE 13</h1>
		...
		'actionButtons' => array(
			...
			array(
				...
				'click' => 'js:function(checked_element){
						...
						showPopupModal("Reject Reason","'.(Yii::app()->getBaseUrl(true).'/index.php?r=inbox/tvd55/AjxPopRejectChecked').'"+temp);	
				}'
			)
		),
		...
		array(
			...
			'buttons'=>array(
		        'approve'=>array(
		        	'label'=>'approve', 
		            'icon' =>'ok',                          
		            'url'  =>'Yii::app()->createUrl("/inbox/tvd55/approve", array("id" => $data->primaryKey))',				// AH : change 
		         ),
		         'reject'=>array(
		         	'label'=>'reject',
		            'icon'=> 'remove',
		            'url' => 'Yii::app()->createUrl("/inbox/tvd55/AjxPopReject", array("id" => $data->primaryKey))',			// AH : change
		            'click'=>'js:function(e){
		            	e.preventDefault();
