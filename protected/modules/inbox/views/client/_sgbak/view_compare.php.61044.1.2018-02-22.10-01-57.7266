	...
	<div class="row-fluid">
		<div class="span6">
			<div class="control-group">
				...
				<div class="controls">
					<?php echo $form->textField($modelCifDetailCurr,'client_title',array('class'=>'span2','readonly'=>'readonly')); ?>
					<?php echo $form->textField($modelClientDetailCurr,'client_name',array('class'=>'span7','readonly'=>'readonly')); ?>
				</div>
			</div>
		</div>
		<div class="span6">
	...
	<?php
		...
		$tabs = array(
			array(
	                ...
	                'content' =>  $this->renderPartial('_view_client_indi',array('modelClientIndiDetail'=>$modelClientIndiDetailCurr,'modelCifDetail'=>$modelCifDetailCurr,'modelClientEmerDetail'=>$modelClientEmerDetailCurr,'form'=>$form),true,false),
	                'active'  => true
	            ),
             array(
	                'label'   => 'Occupation',
	                'content' =>  $this->renderPartial('_view_client_occupation',array('modelClientIndiDetail'=>$modelClientIndiDetailCurr,'modelCifDetail'=>$modelCifDetailCurr,'form'=>$form),true,false),
	                'active'  => false
	            ),
	         array(
		         	'label' => 'Spouse / Parent',
		         	'content' =>  $this->renderPartial('_view_client_spouse',array('modelClientIndiDetail'=>$modelClientIndiDetailCurr,'modelCifDetail'=>$modelCifDetailCurr,'form'=>$form),true,false),
		            'active'  => false
				 ),
			array(
					'label' => 'Fund',
					'content' => $this->renderPartial('_view_client_fund',array('modelClientIndiDetail'=>$modelClientIndiDetailCurr,'modelCifDetail'=>$modelCifDetailCurr,'form'=>$form),true,false),
					'active' => false
			),
			array(
					'label' => 'Bank',
					'content' => $this->renderPartial('_view_client_bank',array('modelClientIndiDetail'=>$modelClientIndiDetailCurr,'modelCifDetail'=>$modelCifDetailCurr,'modelClientBankDetail'=>$modelClientBankDetailCurr,'form'=>$form),true,false),
					'active' => false
			),
			array(
					'label' => 'Instruction and Information',
					'content' => $this->renderPartial('_view_client_instruction',array('modelClientDetail'=>isset($modelClientDetailCurr)?$modelClientDetailCurr:'','modelClientIndiDetail'=>$modelClientIndiDetailCurr,'modelCifDetail'=>$modelCifDetailCurr,'form'=>$form),true,false),
					'active' => false
			),
		);
		...
		if(isset($modelClientDetailCurr))
		{
			$temp = array(
						...
						'content' => $this->renderPartial('_view_client_broker',array('curr'=>true,'modelClientDetail'=>$modelClientDetailCurr,'modelClientIndiDetail'=>$modelClientIndiDetailCurr,'modelCifDetail'=>$modelCifDetailCurr,'form'=>$form),true,false),
						'active' => false
				);
			
			$tabs = array_merge($tabs,array($temp));
	...
	{	
		if($('#clientCurr').length)
		{
  		...
  		$('#cifCurr #tabMenu .tab-content .tab-pane').each(function() {
			...
			if(tabCount == 5 && <?php if(isset($modelClientDetailCurr))echo '0';else echo '1' ?>)
				$(this).find("input").each(function()
				{
					var temp  = $(this).val().trim();
					
					var objUpdate = $("#cif #tabMenu .tab-content .tab-pane:eq("+tabCount+") input").get(ctr+2);
					var temp2 = $(objUpdate).val().trim();
