...
$this->breadcrumbs=array(
	'High Risk Inbox'=>array('index'),
	'Unprocessed High Risk',
);

$this->menu=array(
	array('label'=>'High Risk Inbox', 'itemOptions'=>array('class'=>'nav-header')),
	...
	array('label'=>'List','url'=>Yii::app()->request->baseUrl.'?r=master/highriskname/index','icon'=>'list'),
);
...
<h1>Unprocessed High Risk Names</h1>
		...
		'actionButtons' => array(
			...
			array(
				...
				'click' => 'js:function(checked_element){
						...
						showPopupModal("Reject Reason","'.(Yii::app()->getBaseUrl(true).'/index.php?r=inbox/highriskname/AjxPopRejectChecked').'"+temp);	
				}'
			)
		),
		...
		array(
			...
			'buttons'=>array(
		        'approve'=>array(
		        	'label'=>'approve', 
		            'icon' =>'ok',                          
		            'url'  =>'Yii::app()->createUrl("/inbox/highriskname/approve", array("id" => $data->primaryKey))',				// AH : change 
		         ),
		         'reject'=>array(
		         	'label'=>'reject',
		            'icon'=> 'remove',
		            'url' => 'Yii::app()->createUrl("/inbox/highriskname/AjxPopReject", array("id" => $data->primaryKey))',			// AH : change
		            'click'=>'js:function(e){
		            	e.preventDefault();
