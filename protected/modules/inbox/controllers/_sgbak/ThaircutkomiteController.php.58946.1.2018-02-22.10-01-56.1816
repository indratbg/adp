...
{
	public $table_name = 'T_HAIRCUT_KOMITE';
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Thaircutkomite::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Thaircutkomite::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail  = Thaircutkomite::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Thaircutkomite();
