...
class TcorpactController extends AAdminController
{
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Tcorpact::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Tcorpact::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail  = Tcorpact::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Tcorpact();
