...
{
	public $table_name = 'MST_SECURITIES_LEDGER';
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = SecuritiesLedger::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = SecuritiesLedger::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail  = SecuritiesLedger::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new SecuritiesLedger();
