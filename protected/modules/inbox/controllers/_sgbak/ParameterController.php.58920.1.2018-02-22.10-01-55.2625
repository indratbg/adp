...
class ParameterController extends AAdminController
{
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Parameter::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Parameter::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail  = Parameter::model()->find("rowid ='$model->table_rowid'");	
			}else{
