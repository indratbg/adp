...
class ClientinstitutionaldetailController extends AAdminController
{
	public $table_name = 'MST_CLIENT_AUTHO';
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Clientautho::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Clientautho::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail     = Clientautho::model()->find("rowid ='$model->table_rowid'");	
			}else{
