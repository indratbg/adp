...
class CounterController extends AAdminController
{
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Counter::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Counter::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail     = Counter::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Counter();
