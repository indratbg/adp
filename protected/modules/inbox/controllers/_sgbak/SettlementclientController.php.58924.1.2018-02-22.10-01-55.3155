...
{
	public $table_name = 'MST_SETTLEMENT_CLIENT';
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Settlementclient::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Settlementclient::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail     = Settlementclient::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Settlementclient();
