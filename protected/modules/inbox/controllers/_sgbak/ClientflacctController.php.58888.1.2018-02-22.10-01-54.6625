...
{
	public $table_name = 'MST_CLIENT_FLACCT';
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Clientflacct::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Clientflacct::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail     = Clientflacct::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Clientflacct();
	...
	public function actionIndex()
	{
		...
		// [AR] Restricted so they can't see close Investor account
		$model->status = '<> C';
