...
class MaplkController extends AAdminController
{
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Maplk::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Maplk::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail  = Maplk::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Maplk();
