...
{
	public $table_name = 'MST_CALENDAR';
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Bourseholidaycalendar::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Bourseholidaycalendar::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail     = Bourseholidaycalendar::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Bourseholidaycalendar();
