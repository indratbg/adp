...
{
	public $table_name = 'T_BELANJA_MODAL';
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Tbelanjamodal::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Tbelanjamodal::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail  = Tbelanjamodal::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Tbelanjamodal();
