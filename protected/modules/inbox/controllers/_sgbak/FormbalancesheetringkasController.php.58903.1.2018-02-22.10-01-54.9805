...
{
	public $table_name = 'FORM_BALANCE_SHEET_RINGKAS';
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Formbalancesheetringkas::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Formbalancesheetringkas::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail  = Formbalancesheetringkas::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Formbalancesheetringkas();
