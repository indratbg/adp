...
class ContractintrabrokerController extends AAdminController
{
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_CONTR_UPD){
			$modelDetail     = Tcontracts::model()->find("rowid ='$model->table_rowid'");	// left
			if($modelDetail->sell_broker_cd){
				$getbrokname = Broker::model()->findByPk(trim($modelDetail->sell_broker_cd));
				$modelDetail->sell_brokname = $getbrokname->broker_name;
			}
			if($modelDetail->buy_broker_cd){
				$getbrokname = Broker::model()->findByPk(trim($modelDetail->buy_broker_cd));
				$modelDetail->buy_brokname = $getbrokname->broker_name;
			}
			
			$modelDetailUpd  = Tcontracts::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			Ttempdetail::generateModelAttributes2($modelDetailUpd, $listttempdetail);
			if($modelDetailUpd->sell_broker_cd){
				$getbrokname = Broker::model()->findByPk(trim($modelDetail->sell_broker_cd));
				$modelDetailUpd->sell_brokname = $getbrokname->broker_name;
			}
			if($modelDetailUpd->buy_broker_cd){
				$getbrokname = Broker::model()->findByPk(trim($modelDetail->buy_broker_cd));
				$modelDetailUpd->buy_brokname = $getbrokname->broker_name;
			}
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail  = Tcontracts::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Tcontracts();
	...
	public function actionApprove($id)
	{
		...
		if($model->status == AConstant::INBOX_CONTR_UPD)
			$model->cancelExistingTcontracts();
		$model->approveTcontracts();
