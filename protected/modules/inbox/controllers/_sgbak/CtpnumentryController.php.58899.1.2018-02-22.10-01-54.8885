...
class CtpnumentryController extends AAdminController
{
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Tbondtrx::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Tbondtrx::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail  = Tbondtrx::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Tbondtrx();
