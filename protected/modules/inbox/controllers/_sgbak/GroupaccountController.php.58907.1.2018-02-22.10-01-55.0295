...
{
	public $table_name = 'MST_GROUP_ACCOUNT';
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Groupaccount::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Groupaccount::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail  = Groupaccount::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Groupaccount();
