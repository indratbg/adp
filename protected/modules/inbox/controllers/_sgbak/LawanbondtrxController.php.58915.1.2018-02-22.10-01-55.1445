...
{
	public $table_name = 'MST_LAWAN_BOND_TRX';
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = LawanBondTrx::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = LawanBondTrx::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail     = LawanBondTrx::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new LawanBondTrx();
