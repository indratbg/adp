...
{
	public $table_name = 'T_IPO_CLIENT';
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Tipoclient::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Tipoclient::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail  = Tipoclient::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Tipoclient();
