...
{
	public $table_name = 'MST_GRP_EMITENT';
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Grpemitent::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Grpemitent::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail  = Grpemitent::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Grpemitent();
