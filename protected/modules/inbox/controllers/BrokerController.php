...
class BrokerController extends AAdminController
{
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Broker::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Broker::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail     = Broker::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Broker();
