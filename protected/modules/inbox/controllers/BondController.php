...
class BondController extends AAdminController
{
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Bond::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Bond::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail     = Bond::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Bond();
