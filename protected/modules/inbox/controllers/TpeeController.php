...
class TpeeController extends AAdminController
{
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Tpee::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Tpee::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail  = Tpee::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Tpee();
