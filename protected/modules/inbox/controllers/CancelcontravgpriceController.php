...
{
	public $menu_name = 'CANCEL CONTRACT AVG PRICE';
	...
	public function actionView($id)
	{
		...
		$updRecordCount = Tmanydetail::model()->find(array('select'=>'MAX(record_seq) record_cnt','condition'=>"update_date = TO_DATE('$model->update_date','YYYY-MM-DD HH24:MI:SS') AND update_seq =:update_seq AND table_name =:table_name AND upd_status =:upd_status",'params'=>array(':update_seq'=>$model->update_seq,
							':table_name'=>$this->table_name,':upd_status'=>'U')));
		$listTmanyDetail = array();
		$listTmanyDetailCancel = array();
				
		for($x=1;$x<=$updRecordCount->record_cnt;$x++)
		{
			$listTmanyDetail[$x] = Tmanydetail::model()->find("update_date = TO_DATE('$model->update_date','YYYY-MM-DD HH24:MI:SS') AND update_seq =:update_seq AND table_name =:table_name AND record_seq = :record_seq AND upd_status = :upd_status",
									array(':update_seq'=>$model->update_seq,':table_name'=>$this->table_name,':record_seq'=>($x),':upd_status'=>'U'));
		}		
										
		for($x=1;$x<=$updRecordCount->record_cnt;$x++)
		{
			$modelDetail[$x] = Tcontracts::model()->find("rowid = '".$listTmanyDetail[$x]->table_rowid."'");
		}
		$z = 1;
		for($x=$updRecordCount->record_cnt+1;$x<=$recordCount->record_cnt;$x++){
			$listTmanyDetailCancel[$z] = Tmanydetail::model()->find("update_date = TO_DATE('$model->update_date','YYYY-MM-DD HH24:MI:SS') AND update_seq =:update_seq AND table_name =:table_name AND record_seq = :record_seq AND upd_status = :upd_status",
									array(':update_seq'=>$model->update_seq,':table_name'=>$this->table_name,':record_seq'=>($x),':upd_status'=>'C'));
			$z++;
		}
		$z = 1;
		for($x=$updRecordCount->record_cnt+1;$x<=$recordCount->record_cnt;$x++){
			$modelDetailCancel[$z] = Tcontracts::model()->find("rowid = '".$listTmanyDetailCancel[$z]->table_rowid."'");
			$z++;
		}
	...
	{
		$model = $this->loadModel($id);
		$model->approveCancelcontravgprice();
