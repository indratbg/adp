...
{
	public $table_name = 'MST_MAP_MKBD';
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Mapmkbd::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Mapmkbd::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail  = Mapmkbd::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Mapmkbd();
