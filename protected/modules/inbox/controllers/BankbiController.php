...
class BankbiController extends AAdminController
{
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Bankbi::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Bankbi::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail     = Bankbi::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Bankbi();
