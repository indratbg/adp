...
class TcontractsavgpriceController extends AAdminController
{
	...
	public function actionView($id)
	{
		...
		$oldRecordCount = Tmanydetail::model()->find(array('select'=>'MAX(record_seq) record_cnt','condition'=>'update_seq =:update_seq AND table_name =:table_name AND upd_status =:upd_status','params'=>array(':update_seq'=>$model->update_seq,
		...
		for($x=1;$x<=$oldRecordCount->record_cnt;$x++){
			$listTmanyDetailCancel[$x] = Tmanydetail::model()->find('update_seq =:update_seq AND table_name =:table_name AND record_seq = :record_seq AND upd_status = :upd_status',
									array(':update_seq'=>$model->update_seq,':table_name'=>$this->table_name,':record_seq'=>($x),':upd_status'=>'C'));
		}
		
		//var_dump($listTmanyDetailCancel[1]->table_rowid);
		//die();
		for($x=1;$x<=$oldRecordCount->record_cnt;$x++){
			$modelDetailCancel[$x] = Tcontracts::model()->find("rowid = '".$listTmanyDetailCancel[$x]->table_rowid."'");
		}

		//var_dump($modelDetailCancel[1]->contr_dt);
		//die();
