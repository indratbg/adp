...
class SalesController extends AAdminController
{
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Sales::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Sales::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail     = Sales::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Sales();
