...
{
	public $table_name = 'MST_TAX_RATE';
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Taxrate::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Taxrate::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail  = Taxrate::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Taxrate();
