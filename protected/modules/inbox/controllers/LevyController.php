...
class LevyController extends AAdminController
{
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Levy::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Levy::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail  = Levy::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Levy();
