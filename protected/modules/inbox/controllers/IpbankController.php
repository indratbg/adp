...
class IpbankController extends AAdminController
{
    ...
    public function actionView($id)
    {
        ...
        if($model->status == AConstant::INBOX_STAT_UPD){
            $modelDetail     = Ipbank::model()->find("rowid ='$model->table_rowid'"); // left
            $modelDetailUpd  = Ipbank::model()->find("rowid ='$model->table_rowid'"); // right [updated version]
            ...
            if($model->status == AConstant::INBOX_STAT_CAN){
                $modelDetail  = Ipbank::model()->find("rowid ='$model->table_rowid'");    
            }else{
                $modelDetail  = new Ipbank();
