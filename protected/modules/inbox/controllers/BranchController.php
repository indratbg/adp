...
class BranchController extends AAdminController
{
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Branch::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Branch::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail     = Branch::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Branch();
