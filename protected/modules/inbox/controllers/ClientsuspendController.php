...
class ClientsuspendController extends AAdminController
{
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Clientsuspend::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Clientsuspend::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail     = Clientsuspend::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Clientsuspend();
	...
	public function approve(&$model){
		...
		$model->approveSuspend($model->reject_reason);
	...
	private function approvePopChecked($model,$arrid)
	{
		$reject_reason = $model->reject_reason;
		
		foreach($arrid as $id):
			$model = $this->loadModel($id);	
			$model->approveSuspend($reject_reason);
	...
	public function actionApproveChecked()
	{
		...
		{
			$model->attributes = $_POST['Ttempheader'];
			if($model->validate() && $this->approvePopChecked($model,$arrid))
				$is_successsave = true;
		}	
