...
{
	public $menu_name = 'FUND MOVEMENT ENTRY';
		...
		if(isset($_POST['arrid'])):
			...
			{ 
				...
				if($model->error_code < 0){
					...
					$sql="select field_value from t_many_detail where table_name='T_FUND_MOVEMENT' AND UPDATE_SEQ='$id'  AND FIELD_NAME='CLIENT_CD'";
					$client_cd=DAO::queryRowSql($sql);
					$client=$client_cd['field_value'];
					
					$client_approve[] =$client;
					/*
					$savePush = new Tpushclient;
										$savePush->client_cd = $client;
										$savePush->push_date= date('Y-m-d H:i:s');
										$savePush->save();	*/
