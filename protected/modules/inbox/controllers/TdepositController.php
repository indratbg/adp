...
class TdepositController extends AAdminController
{
	...
	public function actionView($id)
	{
		...
		if($model->status == AConstant::INBOX_STAT_UPD){
			$modelDetail     = Tdeposit::model()->find("rowid ='$model->table_rowid'");	// left
			$modelDetailUpd  = Tdeposit::model()->find("rowid ='$model->table_rowid'");	// right [updated version]
			...
			if($model->status == AConstant::INBOX_STAT_CAN){
				$modelDetail  = Tdeposit::model()->find("rowid ='$model->table_rowid'");	
			}else{
				$modelDetail  = new Tdeposit();
