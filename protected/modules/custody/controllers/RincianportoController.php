...
class RincianportoController extends AAdminController
{
	...
	{	
		...
		$url = $model->showReportStat($update_date, $update_seq);	
		
		
		$this->render('print',array('model'=>$model,'url'=>$url));
	
	}
	
	public function actionSaveText($update_date,$update_seq){
		
				
				//find data
				$jumlah_acct = Vlaprincianporto::model()->find("update_date='$update_date' and update_seq='$update_seq' and approved_stat='A' ")->jumlah_acct;
				$sql="sELECT STK_CD||'|'||case when instr(price,'.')>1 then trim(to_char(PRICE,9999999999999999990.9999))
					 ...
					 AS TEXT
					from v_lap_rincian_porto where update_date='$update_date' and update_seq='$update_seq' and approved_stat='A' and rep_type=1 order by stk_cd ";
        		...
        		client001
					 AS TEXT2
					from v_lap_rincian_porto where update_date='$update_date' and update_seq='$update_seq' and approved_stat='A' and rep_type=2 order by stk_cd";
				...
				$kode_ab = strtoupper(strtolower(substr($kode_ab, 0,2)));
				$date = Vlaprincianporto::model()->find("update_date='$update_date' and update_seq='$update_seq' and approved_stat='A' ")->report_date;
				$date = DateTime::createFromFormat('Y-m-d H:i:s',$date)->format('Ymd');		
				$namafile = Sysparam::model()->find("param_id='RINCIAN PORTO' AND PARAM_cD1='FILENAME' ")->dstr1;
				$file_name	= $namafile.$date.'.POR';	
				$file = fopen("upload/rincian_porto/$file_name","w");
				...
				//UPDATE SAVETXT_DATE YANG DI SIMPAN KE TEXT FILE
				$sql="UPDATE INSISTPRO_RPT.LAP_RINCIAN_PORTO SET SAVETXT_DATE=SYSDATE WHERE update_date='$update_date' and update_seq='$update_seq' and approved_stat='A'";	
				$exec = DAO::executeSql($sql);
			
				//DOWNLOAD FILE LTH
				$filename = "upload/rincian_porto/$file_name";
		        ...
		        flush();
				readfile($filename);
				unlink("upload/rincian_porto/$file_name");
				exit;
	...
	public function actionGenerate(){
		...
		$success=false;
		if(isset($_POST['Rptrincianporto']))
		{
