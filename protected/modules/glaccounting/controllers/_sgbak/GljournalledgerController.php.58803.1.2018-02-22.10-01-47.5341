...
*/
				...
				if($valid)
				{
					...
					if($success)
					{
						...
						$this->redirect(array('/glaccounting/Gljournalledger/index'));
					}
					else {
	...
	public function actionUpdate($id)
	{
		...
		if(DateTime::createFromFormat('Y-m-d H:i:s',$modeldetail[$x]->arap_due_date))$modeldetail[$x]->arap_due_date=DateTime::createFromFormat('Y-m-d H:i:s',$modeldetail[$x]->arap_due_date)->format('Y-m-d');
			...
			//tampilkan error untuk update jurnal non AR/AP yang melakukan update tanggal< tanggal hari ini
			$last_date=false;
			foreach($modelreversal  as $row)
			{
				$cek = Client::model()->find("client_cd='$row->sl_acct_cd' ");
				//var_dump($row->doc_date);die();
				if(!$cek && $model->jvch_date != date('Y-m-d') )
				{
					$last_date=true;
					break;
				}
			...
			if($last_date)
			{
					$model->addError('jvch_date','Jika update jurnal, tanggal jurnal harus sama dengan tanggal hari ini');
					$valid = FALSE;
			}
			...
			if($model->executeSpHeader(AConstant::INBOX_STAT_UPD,$menuName) > 0)$success = true;
				...
				/* 02 nov 2016
				 * Untuk jurnal non AR/AP boleh diupdate, untuk tanggal jurnal barunya tidak boleh lebih kecil dari hari ini
				 * dan minimal tanggal hari ini atau lebih besar dari tanggal hari ini dan 
				 * kalau terjadi update tanggal dari jurnal maka tanggal reversal ikutin tanggal jurnal terbaru
				 *  
				 */ 
				 /*
				  * 09 dec 2016
				  * ubah tanggal jurnal menjadi tanggal hari ini jika yang diupdate jurnal yang lebih kecil dari tanggal hari ini
				  */
				...
				{
					$cek = Client::model()->find("client_cd='$row->sl_acct_cd' ");
					if($cek || ($row->doc_date > date('Y-m-d')) )
					{
						$arap_flg=true;
						break;
					}
				...
				if(!$arap_flg)
				{
					$jur_date_reversal = date('Y-m-d');
					//09dec2016
				//	if($model->jvch_date > date('Y-m-d') )
					//{
					//	$jur_date_reversal =$model->jvch_date;
				//	}
					$model->jvch_date = $jur_date_reversal;
				}
				else 
				{
					if(DateTime::createFromFormat('Y-m-d H:i:s', $oldmodel->jvch_date)) $oldmodel->jvch_date=DateTime::createFromFormat('Y-m-d H:i:s', $oldmodel->jvch_date)->format('Y-m-d');
				...
				{ 
					if(($modeldetail[$x]->save_flg == 'Y' || $modeldetail[$x]->old_xn_doc_num !='' ) && $modeldetail[$x]->cancel_flg != 'Y')
					{
						...
						$modeldetail[$x]->doc_date = $jur_date_reversal;
						$modeldetail[$x]->due_date = $jur_date_reversal;
						$modeldetail[$x]->netting_date = $jur_date_reversal;
						$modeldetail[$x]->arap_due_date = $jur_date_reversal;
						$modeldetail[$x]->folder_cd = $model->folder_cd;
	...
	public function actionAjxPopDelete($id)
	{
		...
		if(isset($_POST['Tmanyheader']))
		{
			...
			foreach($modelreversal as $row)
			{
				...
				//18may2016, purpose set date=sysdate for non ar/ap journal
				$jur_date_reversal='';
				foreach($modelreversal  as $row)
				{
					$cek = Client::model()->find("client_cd='$row->sl_acct_cd' ");
	...
	public function actionUpdateRemarks($id)
	{
		...
		if(isset($_POST['Tjvchh']))
		{
			...
			if($valid)
			{
					...
					Yii::app()->user->setFlash('success', 'Successfully update description '.$model->folder_cd);
