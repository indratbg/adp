...
<?php echo $form->textFieldRow($modelfilter,'rep_date',array('label'=>false,'class'=>'span','placeholder'=>'dd/mm/yyyy','style'=>'width:100px;display:none'));?>		
...
if($model){
$no_urut = Tconsoljrn::model()->findBySql("Select max(tal_id) as tal_id from t_consol_jrn where xn_doc_num='$row->xn_doc_num'")->tal_id;	
}
else{
	$no_urut=0;
}
	?>
		...
		<tr id="row<?php echo $x ?>" class="<?php if($row->cancel_flg == 'Y')echo 'markCancel' ?>">
			...
			<td width="60px" class="xn_doc_num">
			<?php echo $form->textField($row,'xn_doc_num',array('id'=>"xn_doc_num_$x",'readonly'=>$row->save_flg !='Y'?'readonly':'','class'=>'span','name'=>'Tconsoljrn['.$x.'][xn_doc_num]'));?>
			</td>
			<td width="80px" class="doc_date">
				<?php echo $form->textField($row,'doc_date',array('id'=>"doc_date_$x",'readonly'=>$row->save_flg !='Y'?'readonly':'','class'=>'span tdate','name'=>'Tconsoljrn['.$x.'][doc_date]'));?>
			...
			<td width="40px">
				<?php echo $form->textField($row,'tal_id',array('id'=>"tal_id_($x+1)",'readonly'=>$row->save_flg !='Y'?'readonly':'','class'=>'span','name'=>'Tconsoljrn['.$x.'][tal_id]'));?>
...
function init(){
	...
	$("#tableGen").children('tbody').children('tr').each(function()
		...
		$(this).children('td.xn_doc_num').children('[type=text]').val($(this).children('td.xn_doc_num').children('[type=text]').val().toUpperCase());
		...
		var curr_val = setting.func.number.removeCommas($(this).children('td.curr_val').children('[type=text]').val()) * 100
		var deb = setting.func.number.removeCommas($(this).children('td.curr_val').children('[type=text]').val()) * 100
		...
		$("#tableGen").find('tbody')
    		.prepend($('<tr>')
				...
				.css('width','15px')
						...
						.attr('class','xn_doc_num')
               		 .append($('<input>')
               		 	.attr('class','span')
               		 	.attr('id','xn_doc_num_1')
               		 	.attr('name','Tconsoljrn[1][xn_doc_num]')
               		 	.attr('onchange','init()')
               		 	.attr('type','text')
               		 	.val($('#xn_doc_num_1').val())
               		)
               		.css('width','50px')
               	)
	...
	function reassignId()
   	{
   		for(x=0;x<rowCount;x++)
   		{
			...
			$("#tableGen tbody tr:eq("+x+") td:eq(1) [type=text]").attr("id","xn_doc_num_"+(x+1));
			$("#tableGen tbody tr:eq("+x+") td:eq(2) [type=text]").attr("id","doc_date_"+(x+1));
			$("#tableGen tbody tr:eq("+x+") td:eq(3) [type=text]").attr("id","tal_id_"+(x+1));
			...
			$("#tableGen tbody tr:eq("+x+") td:eq(1) [type=text]").attr("name","Tconsoljrn["+(x+1)+"][xn_doc_num]");
