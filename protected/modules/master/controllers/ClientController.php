...
class ClientController extends AAdminController
{
	...
	public function actionView($id)
	{
		...
		$this->render('view',array(
			...
			'modelClientIndiDetail'=>$this->loadModelClientindi($model->cifs),
			'modelClientEmerDetail'=>$this->loadModelClientEmergency($model->cifs),
	...
	{
		$model			 = new Client;
		$modelClientindi = new Clientindi;
		$modelCif 		 = new Cif;
		$modelClientBank = array();
		$modelClientEmergency = new Clientemergency;
		...
		if(isset($_POST['Client']['copy_client']))
		{
			...
			$result = DAO::queryRowSql("SELECT * FROM MST_CLIENT WHERE client_cd = '$clientCopy' AND client_type_1 = '".Constanta::CLIENT_TYPE1_INDIVIDUAL."'");
			...
			if($result)
			{
				...
				$resultClientIndi = DAO::queryRowSql("SELECT * FROM MST_CLIENT_INDI WHERE cifs = '$cifCopy'");
				$modelClientindi->attributes = $resultClientIndi;
				$modelClientindi->cifs = '';
				
				$resultClientBank = DAO::queryAllSql("SELECT * FROM MST_CLIENT_BANK WHERE cifs = '$cifCopy' AND approved_stat = 'A'");
				...
				$resultClientEmergency = DAO::queryRowSql("SELECT * FROM MST_CLIENT_EMERGENCY WHERE cifs = '$cifCopy'");
				$modelClientEmergency->attributes = $resultClientEmergency;
				$modelClientEmergency->cifs = '';
			}
		}
		
		$model->client_type_1 = Constanta::CLIENT_TYPE1_INDIVIDUAL;
		$model->biz_type = 'ID';
		$model->stk_exch = 'IDX';
		$model->curr_cd = 'IDR';
		...
		$modelCif->client_type_1 = Constanta::CLIENT_TYPE1_INDIVIDUAL;
		$modelCif->biz_type = 'ID';	
		...
		$menuName = 'INDIVIDUAL CLIENT PROFILE ENTRY';
		...
		if(isset($_POST['Client']))
		{
			...
			if($submit=='render')
			{
				...
				if(!($model->cif_opt == Client::CIF_OPTION_EXISTING && !$_POST['auto_comp_cif']))
				{
					...
					{
						//$model->stop_pay = 1;
						//$modelClientBank[0]->default = TRUE;
						...
						$modelClientindi->fam_financial_company = 'N';
						$modelClientindi->fam_suspended_share = 'N';
						$modelClientindi->other_broker_acc = 'N';
						$modelClientindi->own_public_share = 'N';
					}
					else 
					{
						...
						$modelClientindi = $this->loadModelClientindi($cif);
						$modelClientEmergency = $this->loadModelClientEmergency($cif);
						...
						*/
						...
						//$modelClientBank = Clientbank::model()->findAll();
					}
					...
					/*if($minimumFeeFlg->dflg1 == 'Y')
					{
						$check = DAO::queryRowSql("SELECT TRIM(other_1) coy_client_cd FROM MST_COMPANY");
						
						if($minimumFeeFlg->dflg2 == 'Y')$model->internet_client = 'Y';
						else {
							$model->internet_client = 'N';
						}
			...
			else if($submit=='submit')
			{
				...
				$modelClientindi->attributes 		= $_POST['Clientindi'];
				$modelClientEmergency->attributes	= $_POST['Clientemergency'];
				...
				$valid = $modelClientindi->validate() && $valid;
				$valid = $modelCif->validate() && $valid;
				$valid = $modelClientEmergency->validate() && $valid;
				...
				if($modelClientindi->residence_status != 90)
				{
					if($modelClientindi->residence_status)
						$modelClientindi->residence = Parameter::model()->find("prm_cd_1 = 'HOUSE' AND prm_cd_2 = '$modelClientindi->residence_status'")->prm_desc;
				}
				
				if($modelClientindi->educ_code != 90)
				{
					if($modelClientindi->educ_code)
						$modelClientindi->education = Parameter::model()->find("prm_cd_1 = 'EDUC' AND prm_cd_2 = '$modelClientindi->educ_code'")->prm_desc;
				}

				if($modelClientindi->occup_code != 90)
				{
					if($modelClientindi->occup_code)
						$modelClientindi->occupation = Parameter::model()->find("prm_cd_1 = 'WORK' AND prm_cd_2 = '$modelClientindi->occup_code'")->prm_desc;
				}
				
				if($modelCif->annual_income_cd != 90)
				{
					if($modelCif->annual_income_cd)
						$modelCif->annual_income = Parameter::model()->find("prm_cd_1 = 'INCOME' AND prm_cd_2 = '$modelCif->annual_income_cd'")->prm_desc;
				}
				
				if($modelCif->funds_code != 90)
				{
					if($modelCif->funds_code)
						$modelCif->source_of_funds = Parameter::model()->find("prm_cd_1 = 'FUND' AND prm_cd_2 = '$modelCif->funds_code'")->prm_desc;
				}
				
				if($modelCif->net_asset_cd != 90)
				{
					if($modelCif->net_asset_cd)
						$modelCif->net_asset = Parameter::model()->find("prm_cd_1 = 'IASSET' AND prm_cd_2 = '$modelCif->net_asset_cd'")->prm_desc;
				}
				
				if($modelCif->addl_fund_cd && $modelCif->addl_fund_cd != 'Y')
					$modelCif->addl_fund = Parameter::model()->find("prm_cd_1 = 'ADDINC' AND prm_cd_2 = '$modelCif->addl_fund_cd'")->prm_desc;
				
				if($modelCif->source_addl_fund_cd != 90)
				{
					if($modelCif->source_addl_fund_cd)
						$modelCif->source_addl_fund = Parameter::model()->find("prm_cd_1 = 'FUND' AND prm_cd_2 = '$modelCif->source_addl_fund_cd'")->prm_desc;
				}
				...
				{					
					...
					if($model->client_type_3 != 'M')$model->agreement_no = $model->subrek001;
						...
						$model->agreement_no = $broker->broker_004;
					}
					...
					$modelCif->client_title = $model->client_title;
					...
					{					
						...
						{
							//NEW CIF
							$modelCif->cifs = $modelClientindi->cifs = $modelClientEmergency->cifs = $model->cifs;
							...
							$modelClientindi->emergency_addr1 = $model->def_addr_1;
							$modelClientindi->emergency_addr2 = $model->def_addr_2;
							$modelClientindi->emergency_addr3 = $model->def_addr_3;
							$modelClientindi->emergency_postcd = $model->post_cd;
							$modelClientindi->emergency_hp = $model->rebate_basis;
							...
							if($success && $modelClientindi->executeSp(AConstant::INBOX_STAT_INS,$modelClientindi->cifs,$model->update_date,$model->update_seq,1) > 0)$success = TRUE;
							else {
								$success = FALSE;
							}
							
							if($success && $modelClientEmergency->executeSp(AConstant::INBOX_STAT_INS,$modelClientEmergency->cifs,$model->update_date,$model->update_seq,1) > 0)$success = TRUE;
							else {
								$success = FALSE;
							}
						...
						{
							//EXISTING CIF
							//$modelClientindi->cifs = $modelClientEmergency->cifs = $modelCif->cifs;
							$modelCif->purpose06 = $modelCif->purpose07 = $modelCif->purpose08 = $modelCif->purpose09 = $modelCif->purpose10 = $modelCif->purpose11 = '00';
							...
							if($success && $modelClientindi->executeSp(AConstant::INBOX_STAT_UPD,$modelClientindi->cifs,$model->update_date,$model->update_seq,1) > 0)$success = TRUE;
							else {
								$success = FALSE;
							}
							
							if($success && $modelClientEmergency->executeSp(AConstant::INBOX_STAT_UPD,$modelClientEmergency->cifs,$model->update_date,$model->update_seq,1) > 0)$success = TRUE;
							else {
								$success = FALSE;
							}
					...
					if($success)
					{
						...
						$this->redirect(array('/master/client/index'));
					}
					else {
		...
		$this->render('create',array(
			'model'=>$model,
			'modelClientindi'=>$modelClientindi,
			'modelCif'=>$modelCif,
	...
	{
		$model			 = $this->loadModel($id);
		$modelClientindi = $this->loadModelClientindi($model->cifs);
		$modelCif 		 = $this->loadModelCif($model->cifs);
		$modelClientBank = array();
		$modelClientEmergency = $this->loadModelClientEmergency($model->cifs);
	...
	public function actionUpdateSales($id)
	{
		...
		if(isset($_POST['Client']))
		{
			...
			if($model->executeSpHeader(AConstant::INBOX_STAT_UPD, 'INDIVIDUAL CLIENT PROFILE ENTRY') > 0)$success = TRUE;
	...
	public function loadModelClientindi($id)
	{
		$model=Clientindi::model()->findByPk($id);
		if($model===null)
			throw new CHttpException(404,'The requested page does not exist.');
		return $model;
	}
	...
	{
		$model=Clientemergency::model()->find("Cifs = '$id' and rownum = 1");
		if($model===null)
		{
			$model = new Clientemergency;
			$model->cifs = $id;
		}
		return $model;
	}
