...
<table class="items table table-striped table-bordered table-condensed" id="subrek_table">
	<thead>
		<tr>
			<th colspan="4" style="text-align: center">Pengkinian Data</th>
		</tr>
		<tr>
...
<table class="hidden">
	<tr id="clone_row" class="hidden">
		...
		<td><?php echo $form->dropDownListRow($modelData,'client_cd',CHtml::listData(Client::model()->findAll(array('condition'=>"susp_stat <> 'C' and client_type_1 <> 'B' and client_type_1 <> 'H'",'order'=>'client_cd ASC')), 'client_cd', 'CodeAndNameAndType'),array('label'=>false,'class'=>'dropdownclient')); ?></td>
		<td><?php echo $form->textField($modelData,'subrek_001',array('class'=>'span5 subrek001')); ?></td>
		<td><?php echo $form->checkBox($modelData,'yn_001'); ?></td>
	</tr>
</table>
...
<script>
	...
	$('#btnSubmit').click(function(e){
		e.preventDefault();
		sendAjxFormatXmlData();
		$('#xml-001').hide();
		$('#xml-004').hide();
	});
	
	function sendAjxFormatXmlData()
	{
		$.ajax({
			'type' 	  :'POST',
			'url'  	  :'<?php echo CController::createUrl('sdi/AjxFormatXmlData')?>',
			'data' 	  :{detail:$("#sdidata-form").serialize(),header:$("#sdi-form").serialize()},
			'dataType':'json',
			'success' :function(data){
				if(data.save001)
				{
