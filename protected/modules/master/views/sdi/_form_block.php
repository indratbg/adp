...
<table class="items table table-striped table-bordered table-condensed" id="subrek_table">
	<thead>
		<tr>
			<th colspan="6" style="text-align: center">Block</th>
		</tr>
		<tr>
...
<table class="hidden">
	<tr id="clone_row" class="hidden">
		...
		<td><?php echo $form->dropDownListRow($modelBlock,'client_cd',CHtml::listData(Client::model()->findAll(array('condition'=>"susp_stat <> 'C' and client_type_1 <> 'B' and client_type_1 <> 'H'",'order'=>'client_cd ASC')), 'client_cd', 'CodeAndNameAndType'),array('label'=>false,'class'=>'dropdownclient')); ?></td>
		<td><?php echo $form->textField($modelBlock,'subrek_001',array('size'=>4,'class'=>'span10 subrek001')); ?></td>
		<td><?php echo $form->checkBox($modelBlock,'yn_001'); ?></td>
		<td><?php echo $form->textField($modelBlock,'subrek_004',array('size'=>4,'class'=>'span10 subrek004')); ?></td>
		<td><?php echo $form->checkBox($modelBlock,'yn_004'); ?></td>
	</tr>
</table>
...
<script>
	...
	$('#btnSubmit').click(function(e){
		...
		$('#xml-004').hide();
		e.preventDefault();
		sendAjxFormatXmlBlock();
	});
	
	function sendAjxFormatXmlBlock()
	{
		$.ajax({
			'type' 	  :'POST',
			'url'  	  :'<?php echo CController::createUrl('sdi/AjxFormatXmlBlock')?>',
			'data' 	  :{detail:$("#sdiblock-form").serialize(),header:$("#sdi-form").serialize()},
			'dataType':'json',
			'success' :function(data){
				if(data.save001)
				{
