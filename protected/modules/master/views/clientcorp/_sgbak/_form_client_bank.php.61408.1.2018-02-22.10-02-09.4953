...
<?php
	...
	$bankList = Ipbank::model()->findAll(array('condition'=>"approved_stat = 'A' and bank_stat='A'",'order'=>'bank_cd'));
...
<table id='tableBank' class='table-bordered table-condensed'>
	...
	?>
		<tr id="row<?php echo $x ?>" class="<?php if($row->cancel_flg == 'Y')echo 'markCancel' ?>">
			...
			<td>
				<?php echo $form->dropDownList($row,'bank_cd',CHtml::listData($bankList,'bank_cd','DropDownName'),array('class'=>'span','name'=>'Clientbank['.$x.'][bank_cd]','disabled'=>$row->save_flg!='Y'?'disabled':'')); ?>
