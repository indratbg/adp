...
<?php
	...
	$highRiskName = Highriskname::model()->findAll("kategori = 'CUSTOMER' AND approved_stat = 'A'");
	$highRiskJob = Highriskname::model()->findAll("kategori IN ('JOB','OCCUPATION') AND approved_stat = 'A'");
...
?>
	...
	<div class="row-fluid">
		<div class="span6">
			<div class="control-group">
				...
				<div class="controls">
					<?php echo $form->textField($model,'client_title',array('class'=>'span2','maxlength'=>6,'label'=>false,'placeholder'=>'Title')); ?>
					<?php echo $form->textField($model,'client_name',array('class'=>'span7','maxlength'=>50,'placeholder'=>'Name')); ?>
				</div>
			</div>
		</div>
		<div class="span6">
	...
	<div class="row-fluid">
		<div class="span6">
			<?php echo $form->dropDownListRow($model,'branch_code',Chtml::listData(Branch::model()->findAll(array('condition'=>"approved_stat <> 'C'",'order'=>'brch_name')),'brch_cd', 'brch_name'),array('class'=>'span8','prompt'=>'-Choose Branch-')); ?>		
		...
		<div class="span6">
			<div class="span9">
				<div class="control-group">
					...
					<div class="controls">
						<?php echo $form->textField($model,'commission_per',array('class'=>'span6 tnumber')); ?>
	...
	<div class="row-fluid">
		<div class="span6">
			<?php echo $form->dropDownListRow($model,'rebate_basis',AConstant::$client_rebate_basis); ?>
		</div>
		<div class="span6">
			<div class="control-group">
	...
	<?php
		    ...
		    array(
		        ...
		        'tabs' => array(
		            array(
		                ...
		                'content' =>  $this->renderPartial('_form_client_indi',array('model'=>$model,'modelClientindi'=>$modelClientindi,'modelCif'=>$modelCif,'form'=>$form),true,false),
		                'active'  => false
		            ),
		            array(
		                'label'   => 'Occupation',
		                'content' =>  $this->renderPartial('_form_client_occupation',array('model'=>$model,'modelClientindi'=>$modelClientindi,'modelCif'=>$modelCif,'form'=>$form),true,false),
		                'active'  => false
		            ),
		            array('label' => 'Emergency Address', 'content' => 'Messages Content'),
		            array('label' => 'Institution', 'content' => 'Messages Content'),
		            array('label' => 'Authorized Person', 'content' => 'Messages Content'),
		            
					array(
		                'label'   => 'Bank Account',
		                'content' => 'buncit',// $this->renderPartial('_form_client_bank',array('model'=>$modelClientBank,'form'=>$form),true,false),
		                'active'  => true
		            ),
		        ),
		    )
		...
		$tabs = array(
			array(
	                ...
	                'content' =>  $this->renderPartial('_form_client_indi',array('model'=>$model,'modelClientindi'=>$modelClientindi,'modelCif'=>$modelCif,'modelClientEmergency'=>$modelClientEmergency,'form'=>$form),true,false),
	                'active'  => true
	            ),
             array(
	                'label'   => 'Occupation',
	         ...
	         array(
		         	'label' => 'Spouse / Parent',
		         	'content' =>  $this->renderPartial('_form_client_spouse',array('model'=>$model,'modelClientindi'=>$modelClientindi,'modelCif'=>$modelCif,'form'=>$form),true,false),
		            'active'  => false
				 ),
			array(
					'label' => 'Fund',
					'content' => $this->renderPartial('_form_client_fund',array('model'=>$model,'modelClientindi'=>$modelClientindi,'modelCif'=>$modelCif,'form'=>$form),true,false),
					'active' => false
			),
			array(
					'label' => 'Bank',
					'content' => $this->renderPartial('_form_client_bank',array('model'=>$model,'modelClientindi'=>$modelClientindi,'modelCif'=>$modelCif,'modelClientBank'=>$modelClientBank,'cancel_reason'=>$cancel_reason,'form'=>$form),true,false),
					'active' => false
			),
			array(
					'label' => 'Instruction and Information',
					'content' => $this->renderPartial('_form_client_instruction',array('model'=>$model,'modelClientindi'=>$modelClientindi,'modelCif'=>$modelCif,'form'=>$form),true,false),
					'active' => false
			),
			array(
					'label' => 'Broker Authorization',
					'content' => $this->renderPartial('_form_client_broker',array('model'=>$model,'modelClientindi'=>$modelClientindi,'modelCif'=>$modelCif,'init_deposit_cd'=>$init_deposit_cd,'form'=>$form),true,false),
					'active' => false
			),
		);
...
<script>
	...
	$(document).ready(function()
	{
		...
		<?php foreach($highRiskName as $row){ ?>
			highRiskName[x] = [];
			highRiskName[x]['Name'] = "<?php echo str_replace(array('"',chr(10),chr(13)),array('\"',' ',' '),$row->name) ?>";
			x++;
		<?php } ?>
		
		x = 0;
		<?php foreach($highRiskJob as $row){ ?>
			highRiskJob[x] = [];
			highRiskJob[x]['Name'] = "<?php echo str_replace(array('"',chr(10),chr(13)),array('\"',' ',' '),$row->name) ?>";
			x++;
		<?php } ?>
	...
	$('#Client_rebate_basis').change(function()
	{
		...
		{
			/*$('#Client_def_addr_1').prop('readonly',true).val($('#Clientindi_id_addr').val());
			$('#Client_def_addr_2').prop('readonly',true).val($('#Clientindi_id_klurahn').val());
			$('#Client_def_addr_3').prop('readonly',true).val($('#Clientindi_id_kcamatn').val());*/
			$('#Client_def_addr_1').prop('readonly',false);
			$('#Client_def_addr_2').prop('readonly',false);
			$('#Client_def_addr_3').prop('readonly',false);
			$('#Client_post_cd')/*.prop('readonly',true)*/.val($('#Clientindi_id_post_cd').val());
			$('#Client_def_city').val($('#Clientindi_id_kota').val()).css('cursor','not-allowed');
			$('#Client_def_city option:not(:selected)').prop('disabled', true);
			$('#Client_def_city option:selected').prop('disabled',false);
		}
		else if(rebate == 'KT')
		{
			$('#Client_def_addr_1')/*.prop('readonly',true)*/.val($('#Clientindi_empr_addr_1').val());
			$('#Client_def_addr_2')/*.prop('readonly',true)*/.val($('#Clientindi_empr_addr_2').val());
			$('#Client_def_addr_3')/*.prop('readonly',true)*/.val($('#Clientindi_empr_addr_3').val());
			$('#Client_post_cd')/*.prop('readonly',true)*/.val($('#Clientindi_empr_post_cd').val());
			$('#Client_def_city').val($('#Clientindi_empr_city').val()).css('cursor','not-allowed');
			$('#Client_def_city option:not(:selected)').prop('disabled', true);
			$('#Client_def_city option:selected').prop('disabled',false);
		}
		else
	...
	$('#Clientindi_id_addr, #Clientindi_id_klurahn, #Clientindi_id_kcamatn, #Clientindi_id_post_cd, #Clientindi_id_kota').change(function()
	{
		if($('#Client_rebate_basis').val() == 'ID')
		{
			/*$('#Client_def_addr_1').val($('#Clientindi_id_addr').val());
			$('#Client_def_addr_2').val($('#Clientindi_id_klurahn').val());
			$('#Client_def_addr_3').val($('#Clientindi_id_kcamatn').val());*/
			$('#Client_post_cd').val($('#Clientindi_id_post_cd').val());
			$('#Client_def_city').val($('#Clientindi_id_kota').val());
			$('#Client_def_city option:not(:selected)').prop('disabled', true);
			$('#Client_def_city option:selected').prop('disabled',false);
		}
	});
	
	$('#Clientindi_empr_addr_1, #Clientindi_empr_addr_2, #Clientindi_empr_addr_3, #Clientindi_empr_post_cd, #Clientindi_empr_city').change(function()
	{
		if($('#Client_rebate_basis').val() == 'KT')
		{
			$('#Client_def_addr_1').val($('#Clientindi_empr_addr_1').val());
			$('#Client_def_addr_2').val($('#Clientindi_empr_addr_2').val());
			$('#Client_def_addr_3').val($('#Clientindi_empr_addr_3').val());
			$('#Client_post_cd').val($('#Clientindi_empr_post_cd').val());
			$('#Client_def_city').val($('#Clientindi_empr_city').val());
			$('#Client_def_city option:not(:selected)').prop('disabled', true);
			$('#Client_def_city option:selected').prop('disabled',false);
		}
	});
	
	$("#Cif_cif_name").change(function(){
		highRisk('name',$(this).val());
	});
	
	$('#Clientindi_occup_code').change(function(){
		highRisk('job',$(this).find("option:selected").text());
	});
	
	$('#Clientindi_empr_biz_type').change(function(){
		highRisk('biz',$(this).val());
	});
		
	$('#Clientindi_id_negara, #Cif_country').change(function(){
