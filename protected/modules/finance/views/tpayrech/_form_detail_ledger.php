	...
	<table id='tableDetailLedger' class='table-bordered table-condensed'>
		...
		?>
			<tr id="row<?php echo $x ?>" class="<?php if($row->bank_flg == 'Y')echo 'bankRow' ?>">
				...
				<input type="hidden" class="amount" name="Taccountledger[<?php echo $x ?>][cash_withdraw_amt]" />
				<input type="hidden" class="reason" name="Taccountledger[<?php echo $x ?>][cash_withdraw_reason]" />
			</tr>
		<?php $x++;} ?>
		</tbody>
	</table>
		...
		$("#tableDetailLedger").find('tbody')
    		.append($('<tr>')
    			...
    			.append($('<td>')
               	...
               	.append($('<input>')
               		.attr('class','amount')
               		.attr('name','Taccountledger['+(detailLedgerCount+1)+'][cash_withdraw_amt]')
               		.attr('type','hidden')
               	)
               	.append($('<input>')
               		.attr('class','reason')
               		.attr('name','Taccountledger['+(detailLedgerCount+1)+'][cash_withdraw_reason]')
               		.attr('type','hidden')
               	)       	
    		);
