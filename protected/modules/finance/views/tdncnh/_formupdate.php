...
?>
	...
	<?php echo $form->errorSummary($modelRevJvch); ?>
	<?php echo $form->errorSummary($oldmodel); ?>
	<?php echo $form->errorSummary($modelfolder); ?>
		<?php 
	...
	<div class="row-fluid control-group">
		...
		<div class="span4">
			<?php 
			//echo $form->datePickerRow($model,'dncn_date',array('class'=>'span4','placeholder'=>'dd/mm/yyyy','required'=>'required','disabled'=>true,'style'=>'margin-left:-80px;'));
				echo $form->textFieldRow($model,'dncn_date',array('class'=>'span4','required'=>'required','readonly'=>true,'style'=>'margin-left:-80px;'));
			?>	
		</div>
	...
	<div class="row-fluid control-group">
		<div class="span">
			<?php echo $form->textFieldRow($model,'ledger_nar',array('class'=>'span8','required'=>'required','style'=>'margin-left:-80px;width:545px;'));?>	
		</div>
	</div>
	...
	<table id='tableLedger' class='table-bordered table-condensed' >
		...
		?>
			<tr id="row<?php echo $x ?>" class="<?php if($row->cancel_flg == 'Y')echo 'markCancel' ?>">
				...
				<?php echo $form->textField($row,'manual',array('name'=>'Taccountledger['.$x.'][manual]','style'=>'display:none'));?>
				<?php echo $form->textField($row,'reversal_jur',array('name'=>'Taccountledger['.$x.'][reversal_jur]','style'=>'display:none'));?>
				</td>
				...
				<td width="420px" class="ledger_nar">
...
var x;
	...
	function reassignAttribute()
	{
		for(x = 0; x<rowCount; x++)
		{
			...
			$("#tableLedger tbody tr:eq("+x+") td:eq(1) [type=text]").attr("name","Taccountledger["+(x+1)+"][gl_acct_cd]");
...
function filterSlAcct(obj)
	...
	{ 
		...
		{	$(this).children("td.glAcct").children("select").attr("id","gl_acct_cd_"+x);
			...
			$(this).children("td.ledger_nar").children("[type=text]").attr("name","Taccountledger["+x+"][ledger_nar]");
			x++;
		});
	}
		$('#Tdncnh_ledger_nar').change(function(){
			var ledger_nar=$('#Tdncnh_ledger_nar').val();
		$('#Tdncnh_ledger_nar').val(ledger_nar.toUpperCase());
		});
...
initAutoComplete();
	function initAutoComplete()
	{
		...
		$("#tableLedger").children('tbody').children('tr:not(:first)').each(function()
		{
