...
$this->breadcrumbs = array(
	'List of Cheque' => array('index'),
	'List',
);

$this->menu = array(
	array(
		'label' => 'List of Cheque',
...
<div class="row-fluid">
	<div class="span6">
		...
		<div class="control-group">
			...
			<div class="span3">
				...
				<input type="radio" name="Rptlistofcheque[vch_type]" value="0" <?php echo $model->vch_type=='0'?'checked':'' ?>/> &nbsp; All
			</div>
			<div class="span3">
				 <?php //echo $form->radioButton($model, 'vch_type', array('value' => '1')) . "&nbsp Payment Vch"; ?>
				<input type="radio" name="Rptlistofcheque[vch_type]" value="1" <?php echo $model->vch_type=='1'?'checked':'' ?>/> &nbsp; Payment Vch
			</div>
			<div class="span3">
				<?php //echo $form->radioButton($model, 'vch_type', array('value' => '2')) . "&nbsp Receipt Vch"; ?>
				<input type="radio" name="Rptlistofcheque[vch_type]" value="2" <?php echo $model->vch_type=='2'?'checked':'' ?>/> &nbsp; Receipt Vch
			</div>
		</div>
	<div class="control-group">
			<div class="span5">
	...
	<div class="span6">
			...
			<div class="span5">
				<?php echo $form->textField($model, 'bank_cd', array(
					'class' => 'span7',
				));
				?>
			</div>
...
<script>
	...
	$('#Rptlistofcheque_bgn_date').change(function(){
		$('#Rptlistofcheque_end_date').val($('#Rptlistofcheque_bgn_date').val());
	})
	$('#Rptlistofcheque_bank_cd').change(function(){
		$('#Rptlistofcheque_bank_cd').val($('#Rptlistofcheque_bank_cd').val().toUpperCase());
	});
	
    function getClient()
    {
        var result = [];
        $('#Rptlistofcheque_client_cd').autocomplete(
        {
