...
class GenerateimfileController extends AAdminController
{
    ...
    {
        $model = new Generateimfile('GENERATE IM FILE', 'IM_FILE_CSV_SINARMAS', '');
        ...
        if (isset($_POST['Generateimfile']))
        {
            ...
            if ($model->validate() && $model->executeSp() > 0)
            {
                ...
                {
                    $model->tablename = 'IM_FILE_CSV_LIM';
    ...
    public function getXls($model, $ext)
    {
        ...
        $select_query .= " from IPNEXTG.$model->tablename  where user_id= '$user_id' and rand_value=$model->vo_random_value ";
